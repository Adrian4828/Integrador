...
public class ReporteController extends AbstractPdfView {
    ...
    public void generarReportePDFUsuarios(@RequestBody List<Usuarios> usuarios, HttpServletResponse response) {
        try {
            response.setContentType("application/pdf");
    ...
    public void generarReportePDFCategorias(@RequestBody List<Categoria> categorias, HttpServletResponse response) {
        try {
            ...
            for (Categoria categoria : categorias) {
                table.addCell(String.valueOf(categoria.getIdCategoria()));
    ...
    public void generarReportePDFProveedores(@RequestBody List<Proveedor> proveedores, HttpServletResponse response) {
        try {
            ...
            for (Proveedor proveedor : proveedores) {
                table.addCell(String.valueOf(proveedor.getIdProveedor()));
                table.addCell(proveedor.getNombre());
                table.addCell(proveedor.getDireccion());
    ...
    public ResponseEntity<byte[]> excelProv(@RequestBody List<Proveedor> proveedores) {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            // Crear encabezado
            String[] encabezados = { "ID", "Nombre", "Direccion", "Telefono", "Email"};
            Row headerRow = sheet.createRow(0);
            ...
            for (Proveedor proveedor : proveedores) {
                    ...
                    row.createCell(2).setCellValue(proveedor.getDireccion());
    ...
    public ResponseEntity<byte[]> excelUsuarios(@RequestBody List<Usuarios> usuarios) {
        try (Workbook workbook = new XSSFWorkbook()) {
